...
class IsgPenalty(models.Model):
    ...
    def _get_tariff_for_date(self, evaluation_date, tariff_name):
        """Verilen tarihe uygun ceza tarifesini bul (versiyonlama)"""
        domain = [
            ('name', '=', tariff_name),
            ('valid_from', '<=', evaluation_date),
            ('active', '=', True),
        ]
        tariff = self.env['isg.penalty.tariff'].search(domain, order='valid_from desc', limit=1)
        return tariff
    ...
    def _compute_calculated_amount(self):
        for rec in self:
            ...
            # evaluation_date'e uygun tariffe versiyonunu bul
            tariff = rec.tariff_id
            if rec.compliance_id and rec.compliance_id.evaluation_date:
                # evaluation_date'i Datetime'tan Date'e çevir gerekirse
                eval_date = rec.compliance_id.evaluation_date
                if hasattr(eval_date, 'date'):  # Datetime ise
                    eval_date = eval_date.date()
                versioned_tariff = rec._get_tariff_for_date(eval_date, rec.tariff_id.name)
                if versioned_tariff:
                    tariff = versioned_tariff
            
            amount = tariff.amount_2026 or 0.0
            if tariff.amount_per_employee and rec.employee_count:
                amount = amount * rec.employee_count
            if rec.is_repeat_violation and tariff.repeat_multiplier:
                amount = amount * tariff.repeat_multiplier
